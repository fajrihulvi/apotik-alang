...
class Cproduct extends CI_Controller {
   ...
   foreach ($taxfield as $key => $value) {
    $data[$value] = $this->input->post($value)/100;
			...
			$data['product_location'] 	= $this->input->post('product_location',true);
			...
			$data['tax'] 				= $tax;
			$data['price'] 				= $price;
			$data['product_model'] 		= $this->input->post('type_name',true);
   ...
   $header = array('manufacturer_name','product_name','genetic_name','strength','category_name','manufacturer_price','price'); 
   fputcsv($file, $header);
   if($usersData){
