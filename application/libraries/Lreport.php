...
// Date expire Medicine list
	...
	// Rakit halaman "Obat Mendekati Kedaluwarsa"
	public function near_expiry(){
		$CI =& get_instance();
		$CI->load->model('Reports');
		$CI->load->library('occational');
		$data = array(
				'title' 	  => display('near_expiry_list'),
				'totalnumber' => $CI->Reports->near_expiry_count(),
			);
		$reportList = $CI->parser->parse('report/near_expiry',$data,true);
		return $reportList;
	}
