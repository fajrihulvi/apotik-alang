...
public function getProductList($postData=null){
         ...
         $button .='  <a href="'.$base_url.'Cbarcode/barcode_print/'.$record->product_id.'" class="btn btn-warning btn-xs" data-toggle="tooltip" data-placement="left" title="BarCode"><i class="fa fa-barcode" aria-hidden="true"></i></a>';
 if($this->permission1->method('manage_medicine','update')->access()){
	...
	{
		$query=$this->db->select('manufacturer_information.*,product_information.*')
				->from('product_information')
				->join('manufacturer_information', 'manufacturer_information.manufacturer_id = product_information.manufacturer_id','left')
...
// Invoice Data for specific data
	...
	{
		$query=$this->db->select('b.manufacturer_name,a.product_name,a.generic_name,a.strength,c.category_name,a.manufacturer_price,a.price')
